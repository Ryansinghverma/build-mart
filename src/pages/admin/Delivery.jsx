...
export default function AdminDelivery() {
  const deliverable = MOCK_ORDERS.filter(o => ['accepted', 'out_for_delivery'].includes(o.status))
  ...
  const openAssign = (order) => {
    setModal(order)
    const existing = assignments[order.id]
  ...
  const handleAssign = (e) => {
    ...
    setAssignments(prev => ({ ...prev, [modal.id]: { ...form } }))
    setModal(null)
  ...
  return (
    <div className="space-y-5">
      <div>
        ...
        <p className="text-slate-500 text-sm mt-1">{deliverable.length} orders ready for dispatch</p>
      </div>
      ...
      <div>
        ...
        <div className="space-y-3">
          {deliverable.map(order => {
            const assigned = assignments[order.id]
            ...
            return (
              <div key={order.id} className="card flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div className="flex-1">
                  ...
                  <p className="text-sm text-white">{order.products[0]?.name}</p>
                  <p className="text-xs text-slate-500">{order.dealer} · {date}</p>
                </div>
                ...
                {assigned ? (
                  <div className="flex items-center gap-3">
                    <div className="text-right">
                      ...
                      <p className="text-xs text-slate-500">ETA: {assigned.eta}</p>
                    </div>
      ...
      <Modal open={!!modal} onClose={() => setModal(null)} title={`Assign Delivery — #${modal?.id}`}>
