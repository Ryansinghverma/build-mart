...
export default function DealerDashboard() {
  const { user } = useApp()
  const orders = MOCK_DEALER_ORDERS
              ...
              return (
                <div key={order.id} className="card flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-sm text-white mt-0.5">{order.contractor}</p>
                    <p className="text-xs text-slate-500">{order.products[0]?.name} · {date}</p>
                  </div>
